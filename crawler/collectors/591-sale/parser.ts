...
export function parseSaleItems(payload: unknown): Raw591SaleListing[] {
  ...
  return payload.map((item) => { if (!item || typeof item !== 'object') throw new Error('PARSE_ERROR: invalid listing item'); const raw = item as Record<string, unknown>; return { sourceListingId: stringValue(raw.sourceListingId ?? raw.id ?? raw.houseid), title: stringValue(raw.title), city: stringValue(raw.city), district: stringValue(raw.district), addressText: stringValue(raw.addressText ?? raw.address), totalPriceText: stringValue(raw.totalPriceText ?? raw.price), unitPriceText: stringValue(raw.unitPriceText ?? raw.unitPrice), buildingAreaText: stringValue(raw.buildingAreaText ?? raw.area), roomsText: stringValue(raw.roomsText ?? raw.rooms), hallsText: stringValue(raw.hallsText ?? raw.halls), bathroomsText: stringValue(raw.bathroomsText ?? raw.bathrooms), floorText: stringValue(raw.floorText ?? raw.floor), totalFloorsText: stringValue(raw.totalFloorsText ?? raw.totalFloors), buildingAgeText: stringValue(raw.buildingAgeText ?? raw.age), buildingTypeText: stringValue(raw.buildingTypeText ?? raw.buildingType), parkingText: stringValue(raw.parkingText ?? raw.parking), mrtText: stringValue(raw.mrtText ?? raw.mrt), sourceUrl: stringValue(raw.sourceUrl ?? raw.url), raw }; });
}
